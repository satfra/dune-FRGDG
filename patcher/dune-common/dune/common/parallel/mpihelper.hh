...
namespace Dune
{
  ...
  public:
    ...
    DUNE_EXPORT static FakeMPIHelper& instance_threaded([[maybe_unused]] int argc,
                                              [[maybe_unused]] char** argv)
    {
      return instance();
    }
    ...
    DUNE_EXPORT static FakeMPIHelper& instance_threaded()
    {
      static FakeMPIHelper singleton;
      return singleton;
    }
  ...
  public:
    ...
    DUNE_EXPORT static MPIHelper& instance_threaded(int& argc, char**& argv)
    {
      // create singleton instance
      if (!instance_){
        static std::mutex mutex;
        std::lock_guard<std::mutex> guard(mutex);
        if(!instance_)
          instance_.reset(new MPIHelper(argc,argv, MPI_THREAD_MULTIPLE));
      }
      return *instance_;
    }

    DUNE_EXPORT static MPIHelper& instance_threaded()
    {
      if(!instance_)
        DUNE_THROW(InvalidStateException, "MPIHelper not initialized! Call MPIHelper::instance_threaded(argc, argv) with arguments first.");
      return *instance_;
    }
  ...
  private:
    ...
    //! \brief calls MPI_Init with argc, argv, and as parameters
    MPIHelper(int& argc, char**& argv, int threading_level)
    : initializedHere_(false)
    {
      int wasInitialized = -1;
      MPI_Initialized( &wasInitialized );
      if(!wasInitialized)
      {
        ...
        size_ = -1;
        int provided = -1;
        static int is_initialized = MPI_Init_thread(&argc, &argv, threading_level, &provided);
        if(provided < threading_level)
        {
