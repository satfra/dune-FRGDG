...
namespace Dune
{
  ...
  namespace LegendreStuff
  {
    ...
    public:
      ...
      // value and derivatives up to order o of all polynomials
      template<int o>
      void pdnp (D x, std::array<std::vector<R>,o+1>& derivatives) const
      {
        for(int d = 0; d < o+1; ++d)
          derivatives[d].resize(k+1);

        derivatives[0][0] = 1;
        for(int d = 1; d < o+1; ++d)
          derivatives[d][0] = 0.0;
        derivatives[0][1] = 2*x-1;
        derivatives[1][1] = 2.0;
        for(int d = 2; d < o+1; ++d)
          derivatives[d][1] = 0.0;
        ...
        for (int n=2; n<=k; n++)
        {
          derivatives[0][n] = ((2*n-1)*(2*x-1)*derivatives[0][n-1]-(n-1)*derivatives[0][n-2])/n;
          for(int d = 1; d < o+1; ++d)
            derivatives[d][n] = ((2*n-1)*(2*x-1)*derivatives[d][n-1] + 2*d*(2*n-1)*derivatives[d-1][n-1] - (n-1)*derivatives[d][n-2])/n;
        }
      }
    ...
    public:
      ...
      void pdnp (D x, std::array<std::vector<R>,o+1>& derivatives) const
      {
        pdp(x, derivatives[0],derivatives[1]);
        for(int d = 2; d < o+1; ++d)
        {
          derivatives[d].resize(1);
          derivatives[d][0] = 0.0;
        }
      }
    };
    ...
    */
    ...
    public:
                                    ...
                                    std::vector<typename Traits::RangeType>& value) const
      {
        ...
        for (int j=0; j<d; j++) poly.p(x[j],v[j]);
                        ...
                        std::vector<typename Traits::JacobianType>& value) const      // return value
      {
        ...
        // compute values of 1d basis functions in each direction
        for (int j=0; j<d; j++) poly.pdp(x[j],v[j],a[j]);
                   ...
                   std::vector<typename Traits::RangeType>& out) const 
      {
        ...
        else 
        {
          // resize output vector
          out.resize(size());
          //TODO find a FAST way to do this. seems like a bad idea to use a branching if else construct for something so often used
          const unsigned int maxOrder = *std::max_element(order.begin(), order.end());
          if(maxOrder == 1)
          {
            constexpr int o = 1; 
            std::vector<std::array<std::vector<R>,o+1>> derivatives(d);
           // get all derivatives for each dimension
            for (int j=0; j<d; j++) poly.template pdnp<o>(in[j],derivatives[j]);
            ...
            for (int i=0; i<n; i++)
            {
              ...
              // rest of the product
              for (int l=0; l<d; l++)
                out[i] *= derivatives[l] [order[l]] [alpha[l]];
            }
          }
          else if(maxOrder == 2)
          {
            constexpr int o = 2; 
            // dim x order+1 x legendreOrder
            std::vector<std::array<std::vector<R>,o+1>> derivatives(d);
          ...
          {
            constexpr int o = 3; 
            std::array<std::vector<R>,o+1> def;
            def.fill(std::vector<R>(k+1,0.));
            // dim x order+1 x legendreOrder
            std::vector<std::array<std::vector<R>,o+1>> derivatives(d);
