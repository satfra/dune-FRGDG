...
#include <dune/FRGDG/localoperator/LDG_LOP_ConEq_stat.hh>
...
namespace Dune {
  namespace PDELab {
    /** Spatial local operator for local discontinuous Galerkin method
      ...
      \nabla \cdot \{ F(u,du) \}  = 0 in \Omega

      Where u = (u1,...,um) is the solution with m components and derivative
      terms are calculated using the LDG method.
    ...
    template<typename FEM, typename SET, typename DATA>
      class LDGConservationEqSpatialOperator :
        public NumericalJacobianApplyVolume<LDGConservationEqSpatialOperator<FEM,SET,DATA> >,
        public NumericalJacobianVolume<LDGConservationEqSpatialOperator<FEM,SET,DATA> >,
        public NumericalJacobianApplySkeleton<LDGConservationEqSpatialOperator<FEM,SET,DATA> >,
        public NumericalJacobianSkeleton<LDGConservationEqSpatialOperator<FEM,SET,DATA> >,
        public NumericalJacobianApplyBoundary<LDGConservationEqSpatialOperator<FEM,SET,DATA> >,
        public NumericalJacobianBoundary<LDGConservationEqSpatialOperator<FEM,SET,DATA> >,
        ...
        public InstationaryLocalOperatorDefaultMethods<typename SET::Traits::RF>
    {
      private:
        ...
        static constexpr unsigned int dim = NUMFLUX0::dim;
      ...
      public:
        ...
        LDGConservationEqSpatialOperator(NUMFLUX0& numflux0_, NUMFLUX1& numflux1_, DATA& data_, unsigned int overintegration_=0)
          : numflux0(numflux0_), numflux1(numflux1_), overintegration(overintegration_), cache(20), alpha_overall(1.), data(data_){}
        ...
        void preStep (RF time_, RF dt_, int stages_)
        {
          ...
          numflux0.model().setTime(time_);
          numflux1.model().setTime(time_);
        }

        void postStep()
        {
          numflux0.model().setTime(cur_time + cur_dt);
          numflux1.model().setTime(cur_time + cur_dt);
        }
        ...
        void postStage()
        {
          numflux0.model().setTime(post_stage_time);
          numflux1.model().setTime(post_stage_time);
        }
        ...
        template<typename EG, typename LFSU, typename X, typename LFSV, typename R>
          void alpha_volume (const EG& eg, const LFSU& lfsu, const X& x, const LFSV& lfsv, R& r) const
          {
            ...
            const auto xl1 = data.template viewData<1>(cell);

            // Initialize vectors outside for loop
            Dune::FieldVector<RF,m0> u(0.0);
            Dune::FieldVector<RF,m1> g(0.0);
            ...
            for (const auto& ip : quadratureRule(geo,intorder))
            {
              ...
              for (unsigned int k=0; k<m1; k++) // for all components
                for (unsigned int j=0; j<dgspace.size(); j++) // for all basis functions
                  g[k] += xl1[k*dgspace.size() + j]*phi[j];
              ...
              // flux term
              Dune::FieldMatrix<RF,m0,dim> F(0.);
              numflux0.model().flux(eg,qp,u,g,F);
              
              // diffusion flux
						  Dune::FieldMatrix<RF,m0,dim> A(0.);
							numflux0.model().diffFlux(eg,qp,u,g,A); 

              // source term 
              const auto q = numflux0.model().q(cell,qp,u,g);
              ...
              for (unsigned int k=0; k<dgspace.size(); k++)
                for (unsigned int i=0; i<m0; i++)
                  for (unsigned int j=0; j<dim; j++)
                    r.accumulate(lfsv.child(i),k,-(F[i][j]+A[i][j])*gradphi[k][j]*factor);
            }
          }
        ...
        template<typename IG, typename LFSU, typename X, typename LFSV, typename R>
              ...
              R& r_s, R& r_n) const
          {
            ...
            const auto xl1_s = data.template viewData<1>(cell_inside);
            const auto xl1_n = data.template viewData<1>(cell_outside);

            // Initialize vectors outside for loop
            Dune::FieldVector<RF,m0> u_s(0.0);
            Dune::FieldVector<RF,m0> u_n(0.0);
            Dune::FieldVector<RF,m1> p_s(0.0);
            Dune::FieldVector<RF,m1> p_n(0.0);
            ...
            for (const auto& ip : quadratureRule(geo,intorder))
            {
              ...
              // Evaluate p from inside and outside
              p_s = 0.0;
              p_n = 0.0;
              for (unsigned int i=0; i<m1; i++) // for all components
              {
                for (unsigned int k=0; k<dgspace_s.size(); k++)
                {
                  p_s[i] += xl1_s[i*dgspace_s.size() + k]*phi_s[k];
                  p_n[i] += xl1_n[i*dgspace_n.size() + k]*phi_n[k];
                }
              }

              // Compute numerical flux at the integration point
              Dune::FieldVector<RF,m0> f(0.0);
              numflux0.numericalFlux(cell_inside, iplocal_s,
                  cell_outside, iplocal_n,
                  ig.unitOuterNormal(qp),u_s,u_n,p_s,p_n,axis,
                  f,amax);

              // Compute nonconservative flux at the integration point
              Dune::FieldVector<RF,m0> D_s(0.0);
              Dune::FieldVector<RF,m0> D_n(0.0);
              numflux0.model().numericalNonConFlux(cell_inside, iplocal_s,
                  cell_outside, iplocal_n,
                  ig.unitOuterNormal(qp),u_s,u_n,p_s,p_n,
                  D_s, D_n);
              ...
              for (unsigned int k=0; k<dgspace_s.size(); k++)
                for (unsigned int i=0; i<m0; i++) // loop over all components
                  r_s.accumulate(lfsv_s.child(i),k, (f[i]+D_s[i])*phi_s[k]*factor);
              // loop over all vector-valued basis functions
              for (unsigned int k=0; k<dgspace_n.size(); k++)
                for (unsigned int i=0; i<m0; i++) // loop over all components
                  r_n.accumulate(lfsv_n.child(i),k, (-f[i]+D_n[i])*phi_n[k]*factor);
            }
        ...
        template<typename IG, typename LFSU, typename X, typename LFSV, typename R>
              ...
              R& r_s) const
          {
            ...
            for (const auto& ip : quadratureRule(geo,intorder))
            {
              ...
              Dune::FieldVector<RF, m1> p_n(numflux1.model().g(cell_inside, iplocal_s, p_s));
              ...
              Dune::FieldVector<RF, m0> j(numflux0.model().j(ig.intersection(), qp, u_s, f));
