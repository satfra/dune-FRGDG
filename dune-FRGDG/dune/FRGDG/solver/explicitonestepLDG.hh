...
namespace Dune {
  namespace PDELab {
    ...
    class ExplicitOneStepMethodLDG
    {
    ...
    public:
       ...
       */
      ExplicitOneStepMethodLDG(const TimeSteppingParameterInterface<T>& method_, IGOS& igos_, LS& ls_)
       ...
       * there).
       */
      ExplicitOneStepMethodLDG(const TimeSteppingParameterInterface<T>& method_, IGOS& igos_, LS& ls_, TC& tc_)
        : method(&method_), igos(igos_), ls(ls_), verbosityLevel(1), step(1), D(igos),
      ...
      T apply_LDG (T time, T dt, TrlV& xold, TrlV& xnew, Xpass& xpass)
      {
        DefaultLimiter limiter;
        return apply_LDG(time,dt,xold,xnew,xpass,limiter);
      }
       ...
       * \return time step size
       */
      template<typename Limiter, typename Xpass>
      T apply_LDG (T time, T dt, TrlV& xold, TrlV& xnew, Xpass& xpass, Limiter& limiter)
      {
        std::vector<TrlV*> x(1); // vector of pointers to all steps
        x[0] = &xold;         // initially we have only one
        TstV alpha(igos.testGridFunctionSpace()), beta(igos.testGridFunctionSpace()); // split residual vectors
       ...
       */
      template<typename Limiter, typename Xpass>
      T apply_LDG_verbose (T time, T dt, TrlV& xold, TrlV& xnew, Xpass& xpass, Limiter& limiter)
      {
        // save formatting attributes
        ios_base_all_saver format_attribute_saver(std::cout);
        LocalTag mytag;
